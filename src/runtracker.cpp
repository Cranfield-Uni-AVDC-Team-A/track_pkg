...
#define Max_linear_speed 4 //0.6
#define Min_linear_speed 0.4
#define Min_distance 2.0 //1.5
...
float h_rotation_speed_left = 2.4;
float h_rotation_speed_right = 2.72;
...
public:
    ...
    : it_(nh_)
  {
    ...
    image_sub_ = it_.subscribe("/video_source/raw", 1, 
      &ImageConverter::imageCb, this);
    depth_sub_ = it_.subscribe("/zed2/zed_node/depth/depth_registered", 1, 
      &ImageConverter::depthCb, this);
    pub = nh_.advertise<geometry_msgs::Twist>("/segway/cmd_vel", 1000);
  ...
  void depthCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    if(enable_get_depth)
    {
      ...
      // std::cout <<  "result.y = " << result.y << "  result.height = " << result.height << std::endl;
